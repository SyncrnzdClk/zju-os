...
struct sbiret sbi_ecall(uint64_t eid, uint64_t fid, uint64_t arg0,
  ...
  __asm__ volatile(
      ...
      : [eid] "r"(eid), [fid] "r"(fid), [arg5] "r"(arg5), [arg4] "r"(arg4),
...
struct sbiret sbi_debug_console_write_byte(uint8_t byte) {
  ...
  return sbi_ecall(sbi_debug_console_write_byte_eid,
                   sbi_debug_console_write_byte_fid, byte, 0, 0, 0, 0, 0);
}
...
struct sbiret sbi_system_reset(uint32_t reset_type, uint32_t reset_reason) {
  ...
  return sbi_ecall(sbi_system_reset_eid, sbi_system_reset_fid, reset_type,
...
struct sbiret sbi_set_timer(uint64_t stime_value) {
  ...
  return sbi_ecall(sbi_set_timer_eid, sbi_set_timer_fid, stime_value, 0, 0, 0,
                   0, 0);
}
...
struct sbiret sbi_debug_console_read(uint64_t num_bytes, uint64_t base_addr_lo,
  ...
  return sbi_ecall(sbi_debug_console_read_eid, sbi_debug_console_read_fid,
